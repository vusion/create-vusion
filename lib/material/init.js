...
const downloadFromGit = async function (material, dest, isUser) {
...
const formatTemplatePackage = async function (dest, answers, name, isUser) {
    ...
    if (fs.existsSync(_gitignore))
        await fs.move(_gitignore, path.resolve(dest, '.gitignore')); // No overwrite
...
module.exports = async function init(options = {}, args = {}) {
    ...
    try {
        ...
        if (options.type === 'fullstack') {
            ...
            await downloadFromGit('fullstack', dest, true); // 暂时写死，后续使用 serverTemplateName 的值
            await downloadFromGit(clientTemplateName, path.join(dest, 'client'), true);
            spinner = ora(`Initializing the material...`).start();
            await formatTemplatePackage(dest, { }, options.name, true);
            await formatTemplatePackage(path.join(dest, 'server'), { }, options.name + '-server', true);
            await formatTemplatePackage(path.join(dest, 'client'), { }, options.name + '-client', true);
            spinner.succeed();
        } else {
            if (MATERIAL_SOURCES[options.material] && MATERIAL_SOURCES[options.material].startsWith('http')) {
                ...
                await downloadFromGit(options.material, dest, args.isUser);
            }
