...
const { formatTypes } = require('../../command/type.config');
...
const MATERIAL_SOURCES = {
    ...
    'multifile-block': '@vusion-templates/multifile-block',
    'multifile-component': '@vusion-templates/multifile-component',
};
...
module.exports = async function init(options = {}, args = {}) {
    ...
    if (options.type === 'block' || options.type === 'component' || options.type === 'multifile-block' || options.type === 'multifile-component') {
    ...
    try {
...

        {
            ...
            if (formatTypes.includes(options.type)) // 模板都不转换
